...
describe("getFilterAppliedCount", () => {
  const BASE_FILTER = {
    ...
    parseInitialFilterValue: (browserQueryUrlValue) =>
      browserQueryUrlValue || "",
    renderComponent: ({ name, value, update }) => (
      <input key={name} onChange={update} value={value} />
    )
  };
  ...
  it("returns the count of all changed filters", () => {
    expect(getFilterAppliedCount(FILTERS, { keywords: "", phrases: "2" })).toBe(
      1
    );
    expect(
      getFilterAppliedCount(FILTERS, { keywords: "2", phrases: "2" })
    ).toBe(2);
  });

  it("does not count unchanged filters", () => {
    expect(getFilterAppliedCount(FILTERS, { keywords: "", phrases: "" })).toBe(
      0
    );
  });
