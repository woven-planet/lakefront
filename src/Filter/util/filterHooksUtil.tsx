 ...
 */
export const parseInitialFilterValues = (
  location: Location,
  filters: FilterSet
): FilterValues => {
 ...
 */
export const getCurrentBrowserQueryParams = (
  location: Location,
  excludeKeys?: string[]
): ParsedQuery => {
 ...
 */
export const getFilterBrowserQueryParams = (
  filters: FilterSet,
  values: FilterValues
): ParsedQuery => {
  ...
  Object.keys(filters).forEach((key) => {
    ...
    if (
      !filters[key].isDefaultFilterValue(values[key]) ||
      key === "dateRange"
    ) {
      const filter = filters[key];
