...
describe('IconButton', () => {
    ...
    it(`renders the icon and children in the proper order when position is '${POSITIONS.LEFT}'`, () => {
        const { getByText } = render(
            <IconButton iconName={ICON_NAME} position={POSITIONS.LEFT}>
                <div id={TEXT_ID}>{TEXT_ID}</div>
            </IconButton>
        );
        expect(getByText(ICON_NAME)).toBeInTheDocument();
        expect(getByText(TEXT_ID).parentNode.children[1].id).toBe(TEXT_ID);
    });

    it(`renders the icon and children in the proper order when position is '${POSITIONS.RIGHT}`, () => {
        const { getByText } = render(
            <IconButton iconName={ICON_NAME} position={POSITIONS.RIGHT}>
                <div id={TEXT_ID}>{TEXT_ID}</div>
            </IconButton>
        );
        expect(getByText(ICON_NAME)).toBeInTheDocument();
        expect(getByText(TEXT_ID).parentNode.children[0].id).toBe(TEXT_ID);
    });
});
